// Quartus II Verilog Template
// 4-State Moore state machine

// A Moore machine's outputs are dependent only on the current state.
// The output is written only when the state changes.  (State
// transitions are synchronous.)

module Lamaquina
(
	input	clk, input [31:0]dataa, input reset, input enable, input [7:0]ReadUart,
	output reg [7:0] result, output reg [7:0]SendUart, output reg done
);

	// Declare state register
	reg		[2:0]state;

	// Declare states
	parameter Idle=3'b000, Checksum=3'b001, TimeOut1=3'b010, SendBack=3'b011, TryAlarm=3'b100, Alarm=3'b101, Timer=3'b110;

	//Parameters para o funcionamento da maquina
	reg [7:0]tempo = 8'b0000_0000;
   reg [7:0]timer2 = 8'b0000_0000;
   reg [7:0]timer3 = 8'b0000_0000;

   parameter [7:0]temp = 8'b1111_1111;

   reg [7:0] firstbyte = 8'b1000_0000;
   reg [7:0] secondbyte = 8'b1000_0000;
   reg [7:0] checkdado;
	
	reg [31:0]dataa_old;
	
	reg countbyte = 0; 
   reg countSend = 0;

	// Output depends only on the state
	always @ (state) begin
		case (state)
			Idle:begin
				done <= 0;
			end
			Checksum:begin
			end
			TimeOut1:begin
				result <= 4'b110;
				done <= 1;
			end
			SendBack:begin
				result <= firstbyte;
				done <= 1;
			end
			TryAlarm:begin
				if(timer3 >= temp)begin
					result = 4'b1011; //CODIGO PARA BUG NO BUFFER
					done = 1'b1;
				end
			end
			Alarm:begin
				SendUart = 8'b000;
				result = 3'b111;
				done = 1;
			end
			Timer:begin
				if(tempo <= temp)begin
					done <= 1;
				end
				if(countSend == 0)begin
						SendUart = dataa[7:0];// CODIGO DE REQUISIÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢O VINDA DO NIOS
						countSend = 1;
				end 
			end
		endcase
	end

	// Determine the next state
	always @ (posedge clk or posedge reset) begin
		if (reset)
			state <= Idle;
		else
			case (state)
				Idle:begin
					$display("Idle");
					if (dataa != dataa_old)begin// Se tiver algo na requisiao va para o timer
						state <= Timer;
						$display("Chegou coisa nova no dataa");
					end
					else if (enable)begin// Se tiver algo vindo da UART va para o checksum
						state <= Checksum;
						$display("Enable ligado foi pro checksum");
					end
					else
						state <= Idle; 
				end
				Checksum:begin
					$display ("CheckSum");
					checkdado = firstbyte ^ 8'b0011_0111;

					if (checkdado == secondbyte && firstbyte != 8'b0000_0000)begin // Se o dado estiver correto e for um dado requisitado, va para send back
						state <= SendBack;
						done = 1;
						$display("Checksum deu certo!");
					end
					else if (checkdado != secondbyte)begin// Se o dado vier "corrompido" va para o teste de alarme
						state <= TryAlarm;
						done = 1;
						$display("Checksum deu errado");
					end
					else if (checkdado == secondbyte && firstbyte == 8'b0000_0000)// Se o dado estiver correto e for o alarme va para alarme
						state <= Alarm;
						done = 1;
					else
						state <= Idle;
				end
				TimeOut1:begin
					$display("TimeOut1");
					state <= Idle;
				end
				SendBack:begin
					$display ("SendBack");
            		state <= Idle;
				end
				TryAlarm:begin
					$display ("TryAlarm");
					if (enable)// Se ainda estiver em tempo, e receber algo na UART va para o checksum
						begin
							state <= Checksum;
						end
					else if (timer3 >= temp)// Caso o tempo estoure va para o idle e grave um "bug" no buffer
						begin
							$display("estourou bug timer");
							state <= Idle;
							timer3 = 0;
						end
					else
						begin
							timer3 = timer3 + 1;
							state <= TryAlarm;
						end
				end
				Alarm:begin
					$display ("Alarm");
					state <= Idle;
				end
				Timer:begin
					$display ("Timer");
					
					if ((tempo >= temp))begin
							state <= TimeOut1;
							tempo = 0;
							countSend = 0;
					end
					else if (((enable) && (tempo < temp)))begin
						if (countbyte == 0)begin
							$display("if do firstbyte = %b", countbyte);
							firstbyte = ReadUart;
							countbyte = 1;
							state <= Timer;
						end
						else if (countbyte == 1)begin
							$display("if do secondbyte, count = %b", countbyte);
							secondbyte = ReadUart;
							countbyte = 1'b0;
							state <= Checksum;
							countSend = 0;
						end
						$display ("RecevUart: %b",RecevUart);
						$display ("Firstbyte: %b",firstbyte);
						$display ("secondbyte: %b",secondbyte);
					end
					// Inserting 'else' block to prevent latch inference
					else begin
						tempo <= tempo + 1;
						state <= Timer;
					end
					
				end
			endcase
			
	end

endmodule
